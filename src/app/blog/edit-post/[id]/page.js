...
import {editPostValidationSchema} from "@/utils/validationSchema";
...
export default function EditPost({params}) {
  ...
  const initialValues = {title: postData.title, description: postData.description, body: postData.body, publish: postData.publish};
  ...
  const editPostHandler = async (values) => {
    ...
    if (res.status === 201) {
      ...
      showToastSuccess("تغييرات اعمال شد");
      setTimeout(() => {
        navigate.push("/blog");
      }, 3000);
          ...
          {(props) => {
            return (
              <form onSubmit={props.handleSubmit} className="block mx-auto mt-8 lg:w-1/2 w-full">
                ...
                <div className="mb-5">
                  <label htmlFor="publish" className="block mb-2 text-sm font-medium text-gray-600">
                    وضعیت انتشار
                  </label>
                  <select className="border border-gray-300 text-gray-600 text-sm rounded-lg focus:ring-gray-50 focus:border-gray-50 block w-full p-2.5" {...props.getFieldProps("publish")}>
                    <option value={true} selected={postData.publish === true && true}>
                      انتشار یافتن
                    </option>
                    <option value={false} selected={postData.publish === false && true}>
                      عدم انتشار
                    </option>
                  </select>
                </div>
